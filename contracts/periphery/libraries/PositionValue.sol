...
library PositionValue {
    ...
    function total(
        ...
        (uint256 amount0Fee, uint256 amount1Fee) = fees(positionManager, treasury, tokenId);
    ...
    function fees(INonfungiblePositionManager positionManager, address treasury, uint256 tokenId)
        internal
        view
    ...
    function rewards(INonfungiblePositionManager positionManager, uint256 tokenId)
        internal
        view
        returns(uint256 reward)
    {
        ...
        (
            ...
            ,
            ,
        ) = positionManager.positionRewards(tokenId);

        (uint256 poolRewardGrowthInsideLastX128) =
            _getRewardGrowthInside(
                IUniswapV3Pool(
        ...
        reward =
            FullMath.mulDiv(
                poolRewardGrowthInsideLastX128 - positionRewardGrowthInsideLastX128,
                liquidity,
                FixedPoint128.Q128
            ) +
            rewardsOwed;
    ...
    function _fees(INonfungiblePositionManager positionManager, address treasury, FeeParams memory feeParams)
        private
        view
        returns (uint256 amount0, uint256 amount1)
    {
    ...
    function _getFeeGrowthInside(
        ...
        if (tickCurrent < tickLower) {
            ...
            (uint256 feeGrowthGlobal0X128, uint256 feeGrowthGlobal1X128) = _getFeeGrowthGlobalWithAirdrop(pool, treasury);
    ...
    function _getRewardGrowthInside(
        ...
        (, int24 tickCurrent, , , , , ) = pool.slot0();
        (, , , , , , , , uint256 lowerRewardGrowthOutsideX128) = pool.ticks(tickLower);
        (, , , , , , , , uint256 upperRewardGrowthOutsideX128) = pool.ticks(tickUpper);

        if (tickCurrent < tickLower) {
            rewardGrowthInsideX128 = lowerRewardGrowthOutsideX128 - upperRewardGrowthOutsideX128;
        } else if (tickCurrent < tickUpper) {
            uint256 rewardGrowthGlobalX128 = pool.rewardGrowthGlobalX128();
            rewardGrowthInsideX128 = rewardGrowthGlobalX128 - lowerRewardGrowthOutsideX128 - upperRewardGrowthOutsideX128;
        } else {
            rewardGrowthInsideX128 = upperRewardGrowthOutsideX128 - lowerRewardGrowthOutsideX128;
        }
    }

    function _getFeeGrowthGlobalWithAirdrop(
        IUniswapV3Pool pool,
        address treasury
    ) private view returns (uint256 feeGrowthGlobal0X128, uint256 feeGrowthGlobal1X128) {
        feeGrowthGlobal0X128 = pool.feeGrowthGlobal0X128();
        feeGrowthGlobal1X128 = pool.feeGrowthGlobal1X128();
        (uint256 airdrop0, uint256 airdrop1) = IV3Treasury(treasury).getPoolAirdropAmount(address(pool));
        if (airdrop0 > 0) feeGrowthGlobal0X128 += FullMath.mulDiv(airdrop0, FixedPoint128.Q128, pool.liquidity());
        if (airdrop1 > 0) feeGrowthGlobal1X128 += FullMath.mulDiv(airdrop1, FixedPoint128.Q128, pool.liquidity());
    }
}
